...
def load_smb_racmo(dataset_path,xx,yy,res,time=2015,interp_method='linear',k=1):
    ...
    '''
    mask = np.array([poly.contains(Point(x, y)) for x, y in zip(xx2.flatten(), yy2.flatten())])
    mask = mask.reshape(xx2.shape)
    ...
    ix = xx2[mask]
    iy = yy2[mask]
    iz = ds.isel(time=time_int)['smb'].values.squeeze()[mask]

    '''
    #FIXME Change to allow for non-box shapes/polygons
...
def load_dhdt(dataset_path,xx,yy,res,interp_method='linear',k=1,begin_year=2014,month=5,end_year=2016):
    ...
    #FIXME check if this needs to be changed as well
...
"""
#NOTE used towards end for radar data
...
Returns:
    ...
    TODO: test
"""
#NOTE used at computation heavy portion at end
